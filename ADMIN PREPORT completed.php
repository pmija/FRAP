...
if(isset($_POST['print'])){
    ...
    header("Location: http://".$_SERVER['HTTP_HOST'].  dirname($_SERVER['PHP_SELF'])."/generateCOD.php");
}
...
from loans l 
join (SELECT LOAN_REF from txn_reference join (SELECT max(txn_date) as 'Date' from txn_reference where txn_type = 2 AND SERVICE_TYPE between 3 and 4) latest where txn_date = latest.Date AND SERVICE_TYPE between 3 and 4 AND TXN_TYPE = 2) t
...
where l.LOAN_STATUS = 3";
    ...
    if($_POST['date'] != "0"){
        ...
        $query="SELECT m.member_id as 'ID',m.firstName as 'First',m.middlename as 'Middle', m.lastname as 'Last',l.LOAN_DETAIL_ID as 'Ref',l.LOAN_ID
from loans l 
join (SELECT LOAN_REF 
from txn_reference 
where month(TXN_DATE) = $month AND YEAR(txn_date) = $year AND Day(txn_date) = $day AND SERVICE_TYPE between 3 and 4 AND TXN_TYPE = 2) t 
on l.LOAN_ID = t.LOAN_REF 
join member m 
on l.member_id = m.member_id 
where l.LOAN_STATUS = 3
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-lg-6">
                                       ...
                                       <form action="ADMIN PREPORT completed.php" method="POST">
                                        ...
                                        <?php
                                        $query="SELECT DISTINCT MONTH(txn_date) as 'Month',YEAR(txn_date) as 'Year', Day(txn_date) as 'Day' from txn_reference t join loans l on l.loan_id = t.LOAN_REF
                                            where txn_type = 2 AND SERVICE_TYPE between 3 and 4 AND l.loan_status = 3";
